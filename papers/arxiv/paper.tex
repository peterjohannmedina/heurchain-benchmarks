...
\subsection{The multi-track empirical iteration cycle}
...
The \texttt{all} result is suggestive but the comparison is heavily confounded and we treat it as hypothesis-generating rather than as evidence of a general principle. Despite having access to a fact pool where the gold-bearing detail is present 66.7\% of the time, the answerer wins zero questions and refuses on 11 of 12, while the 20-fact \texttt{event-idx} strategy wins 3/12. Four caveats apply directly. (i) \textbf{$n=12$} on this subset is not statistically powered: a 0/12 vs 3/12 split does not reject the null hypothesis at $p<0.05$ under a binomial test. (ii) \textbf{$k$ is confounded with selection}: \texttt{all} passes 60 facts while \texttt{event-idx} and \texttt{top-k} pass 20, so any portion of the gap may be context-length effect rather than selection quality. The lost-in-the-middle literature \citep{liu2024lost} already documents long-context attention degradation, and our 91.7\% refusal rate at $k=60$ is consistent with that mechanism --- a controlled-$k$ experiment (running \texttt{event-idx} at $k=60$ or \texttt{all} at $k=20$) would be required to attribute the gap to selection rather than to context length. (iii) The 3-vs-2 delta between \texttt{event-idx} and lexical \texttt{top-k} turns on the date-tag signal that \texttt{event-idx} uniquely uses; it does not isolate ``selection quality'' in any general sense. (iv) The v4b answer prompt's refusal heuristic may interact with context length differently than other answer prompts would, and we measure only one answer model (DeepSeek V3.1). The directional pattern is real but the proper framing is: under our specific harness and on this 12-question slice, more context did not help. \Cref{sec:selrecall} develops this as a hypothesis; \Cref{sec:threats} enumerates the threats to validity.
...
\subsection{Selection vs.\ recall: a calibrated observation}
\label{sec:selrecall}

The v4c stage-2 comparison (\Cref{tab:v4c}) is a suggestive but preliminary signal --- not a validated finding --- about RAG-style answering. With identical haystack-wide facts, identical answer prompt (v4b), and identical judge (DeepSeek V3.1), the \texttt{all} strategy (60 facts) returned 0/12 wins and a 91.7\% refusal rate, while the 20-fact \texttt{event-idx} strategy returned 3/12 wins at a 66.7\% refusal rate; the 20-fact lexical baseline returned 2/12. The directional pattern is consistent with an architectural reflex in agent-memory systems that we would like to question: when answers are wrong, increase $k$. Our 12-question slice is \emph{consistent} with the opposite --- that beyond some threshold, additional candidates do not help and may hurt.

We deliberately do not frame this as evidence that ``selection dominates recall.'' Four reasons. (i) $n=12$ is underpowered for a general claim about selection vs.\ recall. (ii) \texttt{all} was run at $k=60$ while \texttt{event-idx} and lexical were run at $k=20$, so context length is confounded with selection --- the \texttt{all} regression is equally consistent with the lost-in-the-middle attention-degradation effect documented by \citet{liu2024lost} as it is with a selection-quality argument. (iii) The 3-vs-2 delta between \texttt{event-idx} and lexical reflects the date-tag signal that the event index uniquely exploits on a date-arithmetic question category, not a general property of ``better selection.'' (iv) We measure a single answer model whose refusal heuristic may interact idiosyncratically with context length.
...
\subsection{Threats to validity (v4c stage-2 selection-vs-recall observation)}
\label{sec:threats}

The ``more context did not help'' observation in \Cref{sec:v4c} and \Cref{sec:selrecall} carries threats that we want to surface explicitly rather than bury in a general limitations list.

\begin{itemize}
  \item \textbf{Underpowered sample.} The v4c stage-2 measurement is on $n=12$ --- the retrieval-miss subset of v4b residual failures within a single category. A 0/12 vs 3/12 split does not reject the null hypothesis at $p<0.05$ under a binomial test. Any general claim about selection vs.\ recall from this slice is statistically unwarranted.
  \item \textbf{$k=60$ vs $k=20$ confound.} The \texttt{all} strategy was run at $k=60$ while \texttt{event-idx} and lexical were run at $k=20$. The observed gap therefore conflates selection quality with raw context length. A fair selection-vs-recall test would run \texttt{event-idx} at $k=60$ (or \texttt{all} at $k=20$) to isolate the effects. We have not yet run that controlled experiment.
  \item \textbf{Lost-in-the-middle as an alternative explanation.} \citet{liu2024lost} document that LLMs attend less reliably to middle positions in long contexts. Our 91.7\% refusal rate at $k=60$ is consistent with that mechanism, which would predict our observation without any selection-quality argument. Our data cannot distinguish the two explanations.
  \item \textbf{Single answer model.} All v4c stage-2 numbers use DeepSeek V3.1 671B as both answerer and judge. The interaction between context length and the v4b refusal heuristic may be specific to this model; behavior may differ for other model classes (e.g., long-context-trained models, models with different refusal calibration).
  \item \textbf{Single benchmark, single category, single subset.} LongMemEval-S temporal-reasoning is one of six categories on one benchmark; the retrieval-miss subset is a slice of one residual. Findings about date-arithmetic questions may not transfer to non-temporal categories or to other benchmark distributions.
  \item \textbf{Selection-signal confound between \texttt{event-idx} and lexical top-$k$.} The event index uses (date-tag filter + entity overlap); the lexical baseline uses (entity overlap only). The 3-vs-2 delta is therefore primarily the date-tag signal --- which is specifically useful on a temporal-reasoning question set --- rather than a general ``selection quality'' effect that would transfer to other question types.
  \item \textbf{Same-author harness; no independent replication.} All measurements are produced by the harness we wrote and run by the author. Independent replication on a different harness implementation would strengthen any of the claims in this paper.
\end{itemize}
...
We presented three methodology contributions for AI-agent memory benchmarking, demonstrated on HeurChain across the v2 $\to$ v4c prompt-and-tool engineering arc on LongMemEval-S. First, \textbf{cross-frontier per-question judge agreement} (87.8\% on DeepSeek + Kimi) is a stronger defensibility metric than any single judge's QA accuracy and exposes 1--7\,pp of same-class judge inflation per category. Second, the \textbf{multi-track empirical iteration cycle} --- separating extraction, answer, and retrieval iteration --- recovers wins that single-track iteration would miss; on the 22 temporal-reasoning failures we converted a 1/22 null result on extraction iteration into 7/22 after answer-prompt iteration and 10/22 (45.5\%) after a third retrieval-side iteration (v4c haystack re-extraction + event index), projecting to $+33$\,pp on the full temporal-reasoning category versus the v3 baseline (see \Cref{fig:cascade}). Third, the \texttt{fact\_contains\_gold} diagnostic identifies the high-Hit@10 / low-QA gap as an extraction-quality problem rather than a retrieval problem, and the v4c stage-1 measurement (66.7\% recoverability haystack-wide) localizes how much of the gap is recall-bound versus selection-bound. We additionally report a preliminary observation that, on the v4c retrieval-miss subset, a 60-fact \texttt{all} dump returned 0/12 wins while a 20-fact event-indexed selection returned 3/12 against an identical answer prompt and judge --- consistent with documented long-context attention effects \citep{liu2024lost}. We frame this as hypothesis-generating only: $n=12$ is underpowered, $k=60$ vs $k=20$ confounds context length with selection, and a controlled-$k$ follow-up is required before any general ``selection vs.\ recall'' claim can be made (see \Cref{sec:threats}).
