...
2. A **multi-track empirical iteration cycle** in which extraction, answer, and retrieval are independent iteration tracks; we show that the v4 extraction prompt produced a null result in isolation, the combined v4 + v4b answer prompt recovered +27.3 pp on the targeted failure set, and a subsequent [v4c]{.new-inline} retrieval-side iteration (haystack-wide re-extraction plus a graph-lite [event index]{.new-inline}) brought the cumulative cascade to 45.5%. All three tracks required independent iteration; single-track methodology would have terminated at the v4 null. We adopt a closed-loop empirical iteration methodology — hypothesize, instrument, modify, measure, repeat — as articulated for prompt engineering in the practitioner community most prominently by Karpathy [2024]; the underlying approach is the scientific method applied to the harness layer.
...
### 3.4 The multi-track empirical iteration cycle
...
The `all` result is suggestive but the comparison is heavily confounded and we treat it as hypothesis-generating rather than as evidence of a general principle. Despite having access to a fact pool where the gold-bearing detail is present 66.7% of the time, the answerer wins zero questions and refuses on 11 of 12, while the 20-fact `event-idx` strategy wins 3/12. Four caveats apply directly. (i) **n=12** on this subset is not statistically powered: a 0/12 vs 3/12 split does not reject the null at p<0.05 under a binomial test. (ii) **k is confounded with selection**: `all` passes 60 facts while `event-idx` and `top-k` pass 20, so any portion of the gap may be context-length effect rather than selection quality. The lost-in-the-middle literature [Liu et al., 2024] already documents long-context attention degradation, and our 91.7% refusal rate at k=60 is consistent with that mechanism — a controlled-k experiment (running `event-idx` at k=60 or `all` at k=20) would be required to attribute the gap to selection rather than to context length. (iii) The 3-vs-2 delta between `event-idx` and lexical top-k turns on the date-tag signal that `event-idx` uniquely uses; it does not isolate "selection quality" in any general sense. (iv) The v4b answer prompt's refusal heuristic may interact with context length differently than other answer prompts would, and we measure only one answer model (DeepSeek V3.1). The directional pattern is real but the proper framing is: under our specific harness and on this 12-question slice, more context did not help. Section 6.5 develops this as a hypothesis; Section 7 enumerates the threats to validity.
...
::: {.new-block}
### 6.5 Selection vs. recall: a calibrated observation

The v4c stage-2 comparison (Table 7) is a suggestive but preliminary signal — not a validated finding — about RAG-style answering. With identical haystack-wide facts, identical answer prompt (v4b), and identical judge (DeepSeek V3.1), the `all` strategy (60 facts) returned 0/12 wins and a 91.7% refusal rate, while the 20-fact `event-idx` strategy returned 3/12 wins at a 66.7% refusal rate; the 20-fact lexical baseline returned 2/12. The directional pattern is consistent with an architectural reflex in agent-memory systems that we would like to question: when answers are wrong, increase k. Our 12-question slice is *consistent* with the opposite — that beyond some threshold, additional candidates do not help and may hurt.

We deliberately do not frame this as evidence that "selection dominates recall." Four reasons. (i) n=12 is underpowered for a general claim about selection vs. recall. (ii) `all` was run at k=60 while `event-idx` and lexical were run at k=20, so context length is confounded with selection — the `all` regression is equally consistent with the lost-in-the-middle attention-degradation effect documented by Liu et al. [2024] as it is with a selection-quality argument. (iii) The 3-vs-2 delta between `event-idx` and lexical reflects the date-tag signal that the event index uniquely exploits on a date-arithmetic question category, not a general property of "better selection." (iv) We measure a single answer model whose refusal heuristic may interact idiosyncratically with context length.

The defensible read is therefore narrower: on a 12-question retrieval-miss subset of LongMemEval-S temporal-reasoning, holding extraction prompt and judge fixed, a date-tagged event index at k=20 outperformed a 60-fact dump at producing committed answers. Whether the effective cause is selection quality, context length, the date-tag signal, or refusal-heuristic interaction — and whether the pattern survives at frontier answer models or on other categories — are open questions. We propose follow-up controlled-k experiments (running each strategy at multiple k values, holding all else fixed) as the natural next step. Treating this section's observation as a hypothesis to test rather than as a result to cite is, in our view, the appropriate calibration.
...
### 7.1 Threats to Validity (v4c stage-2 selection-vs-recall observation)

The "more context did not help" observation in Section 5.6 and Section 6.5 carries threats that we want to surface explicitly rather than bury in a general limitations list.

- **Underpowered sample.** The [v4c]{.new-inline} stage-2 measurement is on n=12 — the retrieval-miss subset of v4b residual failures within a single category. A 0/12 vs [3/12]{.new-inline} split does not reject the null hypothesis at p<0.05 under a binomial test. Any general claim about selection vs. recall from this slice is statistically unwarranted.
- **k=60 vs k=20 confound.** The `all` strategy was run at k=60 while `[event-idx]{.new-inline}` and lexical were run at k=20. The observed gap therefore conflates selection quality with raw context length. A fair selection-vs-recall test would run `[event-idx]{.new-inline}` at k=60 (or `all` at k=20) to isolate the effects. We have not yet run that controlled experiment.
- **Lost-in-the-middle as an alternative explanation.** Liu et al. [2024] document that LLMs attend less reliably to middle positions in long contexts. Our 91.7% refusal rate at k=60 is consistent with that mechanism, which would predict our observation without any selection-quality argument. Our data cannot distinguish the two explanations.
- **Single answer model.** All [v4c]{.new-inline} stage-2 numbers use DeepSeek V3.1 671B as both answerer and judge. The interaction between context length and the v4b refusal heuristic may be specific to this model; behavior may differ for other model classes (e.g., long-context-trained models, models with different refusal calibration).
- **Single benchmark, single category, single subset.** LongMemEval-S temporal-reasoning is one of six categories on one benchmark; the retrieval-miss subset is a slice of one residual. Findings about date-arithmetic questions may not transfer to non-temporal categories or to other benchmark distributions.
- **Selection-signal confound between `[event-idx]{.new-inline}` and lexical top-k.** The [event index]{.new-inline} uses (date-tag filter + entity overlap); the lexical baseline uses (entity overlap only). The 3-vs-2 delta is therefore primarily the date-tag signal — which is specifically useful on a temporal-reasoning question set — rather than a general "selection quality" effect that would transfer to other question types.
- **Same-author harness; no independent replication.** All measurements are produced by the harness we wrote and run by the author. Independent replication on a different harness implementation would strengthen any of the claims in this paper.

We retain the directional observation as hypothesis-generating signal because it explicitly motivates the controlled-k experiments we propose under Future Work. We do not retain it as a contribution.

---

## 8. Future Work

**Beyond [v4c]{.new-inline} — better question parsing and multi-shot retrieval.** [v4c]{.new-inline} (Section 5.6) closed roughly 3/8 of the recoverable retrieval-miss questions (those where `fact_contains_gold = True` on the haystack-wide pool). Two directions remain. First, the [event-index]{.new-inline} selector currently uses a coarse entity-overlap heuristic; a proper noun-phrase extractor over the question text (named-entity recognition, or even a small LLM call dedicated to question parsing) should let the selector match richer phrases like "Ancient Civilizations exhibit at the Met" rather than relying on token-bag overlap. Second, "between A and B" questions are intrinsically two-event, but the index is queried in a single shot; a decomposed two-shot retrieval — issue one query per named event, union the results, then pass — should pick up the multi-event class explicitly. The remaining 4/12 questions where `fact_contains_gold` is False even on the haystack-wide pool suggest a separate residual: some events apparently require either session-metadata threading (linking implicit references across sessions) or a different extraction approach that does not depend on the speaker mentioning a date at all.
...
**Multi-event retrieval.** Beyond [v4c]{.new-inline}'s [event index]{.new-inline}, the "between A and B" failure pattern suggests a more general multi-evidence retrieval primitive: when a query implicates two named entities or events, retrieve top-k for each independently and union, rather than retrieving a single top-k from the joint query embedding.

---

## 9. Conclusion

We presented three methodology contributions for AI-agent memory benchmarking, demonstrated on HeurChain across the v2 → [v4c]{.new-inline} prompt-and-tool engineering arc on LongMemEval-S. First, **cross-frontier per-question judge agreement** (87.8% on DeepSeek + Kimi) is a stronger defensibility metric than any single judge's QA accuracy and exposes 1–7 pp of same-class judge inflation per category. Second, the **multi-track empirical iteration cycle** — separating extraction, answer, and retrieval iteration — recovers wins that single-track iteration would miss; on the 22 temporal-reasoning failures we converted a 1/22 null result on extraction iteration into 7/22 after answer-prompt iteration and 10/22 (45.5%) after a third retrieval-side iteration ([v4c]{.new-inline} [haystack re-extraction]{.new-inline} + [event index]{.new-inline}), projecting to **+33 pp on the full temporal-reasoning category** versus the v3 baseline. Third, the **`fact_contains_gold` diagnostic** identifies the high-Hit@10 / low-QA gap as an extraction-quality problem rather than a retrieval problem, and the [v4c]{.new-inline} stage-1 measurement (66.7% recoverability haystack-wide) localizes how much of the gap is recall-bound versus selection-bound. We additionally report a preliminary observation that, on the [v4c]{.new-inline} retrieval-miss subset, a 60-fact `all` dump returned 0/12 wins while a 20-fact event-indexed selection returned [3/12]{.new-inline} against an identical answer prompt and judge — consistent with documented long-context attention effects [Liu et al., 2024]. We frame this as hypothesis-generating only: n=12 is underpowered, k=60 vs k=20 confounds context length with selection, and a controlled-k follow-up is required before any general "selection vs. recall" claim can be made (see Section 7.1).

**[Table 9]{.new-inline}: Cumulative four-layer cascade on temporal-reasoning (22 v3 DeepSeek failures).**
...
| + [v4c]{.new-inline} [event-index]{.new-inline} retrieval                |         10 / 22 |              45.5%  |

The contributions are deliberately tool-shaped: the cross-judge runner, the triage classifier, the `fact_contains_gold` heuristic, the [haystack re-extractor]{.new-inline}, and the [event index]{.new-inline} are each a few hundred lines of Python, run in minutes-to-hours against cached outputs, and apply to any memory system that exposes a retrieved-facts cache. We hope they sharpen the conversation about how AI-agent memory systems are evaluated.
...
- `[haystack_reextract]{.new-inline}.py` — [v4c]{.new-inline} stage 1: re-extract v4 facts from every haystack session for a failing question
- `[build_event_index.py]{.new-inline}` — [v4c]{.new-inline} stage 2: build a SQLite + FTS5 [event index]{.new-inline} over `[date: YYYY-MM-DD]`-tagged facts
- `[test_haystack_answer.py]{.new-inline}` — [v4c]{.new-inline} stage 2: answerer harness with `all` / `top-k` / `[event-idx]{.new-inline}` selection strategies
...
The [v4c]{.new-inline} per-record results are at `results/[v4c]{.new-inline}/v4c_all_test.json`, `results/[v4c]{.new-inline}/v4c_topk_test.json`, and `results/[v4c]{.new-inline}/v4c_eventidx_test.json`.
...
9. **Lost in the Middle** — Liu, N. F., Lin, K., Hewitt, J., Paranjape, A., Bevilacqua, M., Petroni, F., Liang, P. (2024). *Lost in the Middle: How Language Models Use Long Contexts.* Transactions of the Association for Computational Linguistics. arXiv:2307.03172.

10. **Karpathy** — Karpathy, A. (2024). *Practitioner notes on iterative prompt engineering and LLM evaluation.* Public communications via X/Twitter and conference talks. https://x.com/karpathy.
